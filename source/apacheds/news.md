...
NOTE: On the latest Apache DS release (2.0.0.AM27), the attached packages have been created with the wrong version (2.0.0.AM28-SNAPSHOT).
Those packages are really for 2.0.0.AM27, but where built using the Apache Jenkins service, which kicked in *after* the release, when the version was already bumped to AM28. This will be fixed in thes next release. Sorry for the inconvenience.
